...
jq -r 'keys[]' "$1" | while read -r key; do
  ...
  ct2-transformers-converter --model Helsinki-NLP/opus-mt-${key}-en --output_dir ./models/${key}_en --force

  # Convert models from language to english
  echo "Downloading and converting: en-${key}..."
  ct2-transformers-converter --model Helsinki-NLP/opus-mt-en-${key} --output_dir ./models/en_${key} --force
